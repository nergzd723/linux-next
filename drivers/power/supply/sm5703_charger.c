				   ...
				   union power_supply_propval *val) 
{
    ...
    switch (psp) {
        case POWER_SUPPLY_PROP_PRESENT:
            error = regmap_read(drv->sm5703->regmap, SM5703_STATUS2, &value);
            val->intval = (value & BIT(4)) ? 0 : 1;
            break;
        case POWER_SUPPLY_PROP_ONLINE:
            error = regmap_read(drv->sm5703->regmap, SM5703_STATUS5, &value);
            val->intval = value & SM5703_STATUS5_VBUSOK ? 1 : 0;
            break;
        case POWER_SUPPLY_PROP_STATUS:
            error = regmap_read(drv->sm5703->regmap, SM5703_STATUS3, &value);
...
static int sm5703_charger_probe (struct platform_device *pdev) {
    ...
    error = regmap_update_bits(drv->sm5703->regmap, SM5703_CNTL, SM5703_AUTOSET_MASK, 
                            (drv->use_autoset << 4));
    if (error)
