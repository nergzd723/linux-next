				   ...
				   union power_supply_propval *val) 
{
    ...
    switch (psp) {
        case POWER_SUPPLY_PROP_TEMP:
            error = regmap_read(drv->regmap, SM5703_REG_TEMPERATURE, &value);
            if (error < 0)
		        return error;
            val->intval = FIXED_POINT_8_8_EXTEND_TO_INT(value, EXTEND_DECI);
            break;
        case POWER_SUPPLY_PROP_CAPACITY:
            error = regmap_read(drv->regmap, SM5703_REG_SOC, &value);
            if (error < 0)
		        return error;
            val->intval = FIXED_POINT_8_8_EXTEND_TO_INT(value, EXTEND_NONE);
            break;
        case POWER_SUPPLY_PROP_VOLTAGE_NOW:
            error = regmap_read(drv->regmap, SM5703_REG_VOLTAGE, &value);
            if (error < 0)
		        return error;
            val->intval = FIXED_POINT_8_8_EXTEND_TO_INT(value, EXTEND_MICRO);
            break;
        case POWER_SUPPLY_PROP_VOLTAGE_OCV:
            error = regmap_read(drv->regmap, SM5703_REG_OCV, &value);
            if (error < 0)
		        return error;
            val->intval = FIXED_POINT_8_8_EXTEND_TO_INT(value, EXTEND_MICRO);
            break;
        case POWER_SUPPLY_PROP_CURRENT_NOW:
            error = regmap_read(drv->regmap, SM5703_REG_CURRENT, &value);
            if (error < 0)
		        return error;
            value &= 0x7ff;
